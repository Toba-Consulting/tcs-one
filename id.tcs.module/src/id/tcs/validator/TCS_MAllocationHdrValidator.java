	...
	private static String getMatchAllocationDescription(X_T_MatchAllocation match){
		...
		if (C_Payment_ID>0 && C_Charge_ID>0) {
			...
			if (allocateCharge_ID>0) {
				...
				MPayment pay = new MPayment(match.getCtx(), C_Payment_ID, match.get_TrxName());
		...
		//Case 2
			...
			if (C_AllocationHdr_ID>0) {
				MAllocationHdr allocHdr = new MAllocationHdr(match.getCtx(), C_AllocationHdr_ID, match.get_TrxName());
		...
		//Case 3
			...
			MInvoice inv = new MInvoice(match.getCtx(), C_Invoice_ID, match.get_TrxName());
		...
		//Case 4
			...
			MInvoice inv = new MInvoice(match.getCtx(), Match_Invoice_ID, match.get_TrxName());
			return inv.getDescription();
		}
