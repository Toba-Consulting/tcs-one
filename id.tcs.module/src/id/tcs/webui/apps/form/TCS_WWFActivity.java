 ...
 *
 */
public class TCS_WWFActivity extends ADForm implements EventListener<Event>
{
	/**
	 * 
	 */
	...
	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(TCS_WWFActivity.class);
	...
	public void onOK()
	{
		...
		try {
			...
			if (forward != null)
			{
				...
				{
					//@PhieAlbert
					String whereClause = "AD_Client_ID=? AND AD_WF_Activity_ID = ? AND isApproved=?";
					int[] AD_WF_ActivityApprover_ID = new Query(Env.getCtx(), X_AD_WF_ActivityApprover.Table_Name, whereClause, trx.getTrxName())
													.setParameters(new Object[]{Env.getAD_Client_ID(Env.getCtx()), m_activity.getAD_WF_Activity_ID(), false})
													.setOrderBy(X_AD_WF_ActivityApprover.COLUMNNAME_Sequence)
													.getIDs();
					
					for(int id : AD_WF_ActivityApprover_ID)
					{
						X_AD_WF_ActivityApprover activityApprover = new X_AD_WF_ActivityApprover(Env.getCtx(), id, trx.getTrxName());
						if(activityApprover.getAD_User_ID() == AD_User_ID)
						{
							MUser oldUser = new MUser(Env.getCtx(), AD_User_ID, trx.getTrxName());
							MUser newUser = new MUser(Env.getCtx(), fw, trx.getTrxName());
							
							String descUser = "[" + oldUser.getName() + "=>" + newUser.getName()+ "]: ";
							StringBuilder msg = new StringBuilder();
							if(activityApprover.get_ValueAsString("Description").equals(""))
								msg.append(descUser.concat(textMsg));
							else 
								msg.append(activityApprover.get_ValueAsString("Description"))
								.append("\n").append(descUser).append(textMsg);
							
							activityApprover.setAD_User_ID(fw);
							activityApprover.set_ValueOfColumn("Description", msg.toString());
							activityApprover.saveEx();
						}
					}
					//end @PhieAlbert
				}
			...
			else if (MWFNode.ACTION_UserChoice.equals(node.getAction()))
			{
				...
				try
				{
					if(value.equals("N"))
					{
						...
						//@PhieAlbert
						String whereClause = "AD_Client_ID=? AND AD_WF_Activity_ID = ? AND AD_User_ID = ?";
						int AD_WF_ActivityApprover_ID = new Query(Env.getCtx(), X_AD_WF_ActivityApprover.Table_Name, whereClause, trx.getTrxName())
														.setParameters(new Object[]{Env.getAD_Client_ID(Env.getCtx()), m_activity.getAD_WF_Activity_ID(), AD_User_ID})
														.setOrderBy(X_AD_WF_ActivityApprover.COLUMNNAME_Sequence)
														.firstId();
						
						MUser user = new MUser(Env.getCtx(), AD_User_ID, trx.getTrxName());
						String descUser = "[" + user.getName() + "]: ";
						
						X_AD_WF_ActivityApprover activityApprover = new X_AD_WF_ActivityApprover(Env.getCtx(), AD_WF_ActivityApprover_ID, trx.getTrxName());
						...
						activityApprover.set_ValueOfColumn("Description", msg.toString());
						activityApprover.set_ValueOfColumn("Processed", "Y");
						activityApprover.saveEx();
						//end @PhieAlbert
					}
					else
					{
						...
						for(int id : AD_WF_ActivityApprover_ID)
						{
							...
							if(activityApprover.getAD_User_ID() == AD_User_ID)
							{
								if(count == AD_WF_ActivityApprover_ID.length)
									m_activity.setUserChoice(AD_User_ID, value, dt, "");
								...
								activityApprover.setIsApproved(true);
								activityApprover.set_ValueOfColumn("Description", msg.toString());
								activityApprover.set_ValueOfColumn("Processed", "Y");
								activityApprover.saveEx();
