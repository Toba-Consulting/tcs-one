...
public class TCS_CreateMatchAllocationByCreatedDate extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		//For Delete Existing Match
		String deleteIDs = "";
		for (int i : AllocationIDs) {
			deleteIDs+=i;
			deleteIDs+=", ";
		}
		deleteIDs = deleteIDs.substring(0, deleteIDs.length()-2);
		deleteExistingMatchAllocation(deleteIDs);
...
//			int chargeID = 0;
			...
			for(MAllocationLine line : alloc.getLines(true)){
				...
				if(line.getC_Payment_ID()==0 && line.getC_Invoice_ID()>0 && line.getAmount().compareTo(Env.ZERO)>0){					//1
					...
					X_T_MatchAllocation match = new X_T_MatchAllocation(alloc.getCtx(), 0, alloc.get_TrxName());
					match.set_CustomColumn("C_AllocationHdr_ID", alloc.get_ID());
					...
					MInvoice invoice = new MInvoice(alloc.getCtx(), line.getC_Invoice_ID(), alloc.get_TrxName());
					match.set_CustomColumn("C_DocType_ID", invoice.getC_DocType_ID());
					
					//payment
					match.set_CustomColumn("C_Payment_ID", line.getC_Payment_ID());
					
					MPayment payment = new MPayment(alloc.getCtx(), line.getC_Payment_ID(), alloc.get_TrxName());
					match.set_CustomColumn("Match_DocType_ID", payment.getC_DocType_ID());

					match.set_ValueOfColumn("DateAllocated", alloc.getCreated());
...
//					match.set_CustomColumn("Description", invoice.getDescription());
				...
				if(!plusInvoiceID.isEmpty() && !minInvoiceID.isEmpty()){
					...
					for(int j = 0;j<minInvoiceID.size();j++){
						if(j==0 && minAmount.get(j).compareTo(Env.ZERO)<0){
							...
							MInvoice pInvoice = new MInvoice(alloc.getCtx(), plusInvoiceID.get(i), alloc.get_TrxName());
							...
							MInvoice nInvoice = new MInvoice(alloc.getCtx(), minInvoiceID.get(j), alloc.get_TrxName());
							...
							//match1
							X_T_MatchAllocation match1 = new X_T_MatchAllocation(alloc.getCtx(), 0, alloc.get_TrxName());
							match1.set_CustomColumn("C_AllocationHdr_ID", alloc.get_ID());
				...
				if(!paymentID.isEmpty() && !receiptID.isEmpty()){
					...
					for(int j = 0;j<receiptID.size();j++){
						...
						if(j==0 && receiptAmount.get(j).compareTo(Env.ZERO)>0){
							...
							MPayment pPayment = new MPayment(alloc.getCtx(), paymentID.get(i), alloc.get_TrxName());
							match.set_CustomColumn("C_DocType_ID", pPayment.getC_DocType_ID());
							
							//receipt
							match.set_CustomColumn("Match_Payment_ID", receiptID.get(j));
							
							MPayment nPayment = new MPayment(alloc.getCtx(), receiptID.get(j), alloc.get_TrxName());
				...
				if(!chargeID.isEmpty()){
					if(totalReceiptAmount.compareTo(Env.ZERO)==0 && chargeID.get(i)>0 && tempPaymentAmt.compareTo(Env.ZERO)>0 && chargeAmount.compareTo(Env.ZERO)>0){					
						...
						if (allocateChargeID.get(i)>0) {
							MTCS_AllocateCharge allocateCharge = new MTCS_AllocateCharge(alloc.getCtx(), allocateChargeID.get(i), alloc.get_TrxName());
			...
			for(int i=0; i<minI; i++)
			{
				...
				if(!chargeID.isEmpty()){
					if(tempnInvoiceAmt.compareTo(Env.ZERO)<0 && chargeID.get(0)>0 && chargeAmount.compareTo(Env.ZERO)<0){
						...
						MInvoice nInvoice = new MInvoice(alloc.getCtx(), minInvoiceID.get(i), alloc.get_TrxName());
			...
			for(int i=0; i<rec; i++)
			{
				...
				if(!chargeID.isEmpty()){
					if(tempReceiptAmt.compareTo(Env.ZERO)!=0 && chargeID.get(i)>0 && chargeAmount.compareTo(Env.ZERO)<0){
						...
						MPayment nPayment = new MPayment(alloc.getCtx(), receiptID.get(i), alloc.get_TrxName());
	...
	/*
	 * deleteExistingMatchAllocation
	 * delete match allocation by allocationHdr IDs
	 * format "XXX, XXX, XXX"
	 */
	private void deleteExistingMatchAllocation(String matchAllocationIDs){
		
		String sqlDelete = "DELETE FROM T_MatchAllocation WHERE C_AllocationHdr_ID IN ("+matchAllocationIDs+")";
