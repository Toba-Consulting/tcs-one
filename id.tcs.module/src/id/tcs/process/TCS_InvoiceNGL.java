 ...
 */
public class TCS_InvoiceNGL extends SvrProcess
{
	...
	{
		if (p_AD_Org_ID==0) {
			throw new AdempiereException("Organization Is Mandatory");
		}
		...
		sql = new StringBuilder("INSERT INTO T_InvoiceGL (AD_Client_ID, AD_Org_ID, IsActive, Created,CreatedBy, Updated,UpdatedBy,")
			...
			.append( getAD_PInstance_ID()).append(", i.C_Invoice_ID, i.multiplier*i.GrandTotal, i.multiplier*invoiceOpenToDate(i.C_Invoice_ID, 0,"+dateStr+"), ")
		    ...
		    .append(" INNER JOIN Fact_Acct fa ON (fa.AD_Table_ID=318 AND fa.Record_ID=i.C_Invoice_ID")
		    	.append(" AND (i.multiplier*i.GrandTotal=fa.AmtSourceDr OR i.multiplier*i.GrandTotal=fa.AmtSourceCr) AND fa.Line_ID IS NULL)")
		    .append(" INNER JOIN C_AcctSchema a ON (fa.C_AcctSchema_ID=a.C_AcctSchema_ID) ")
		    .append("WHERE i.multiplier*InvoiceOpenToDate(i.C_Invoice_ID,0, "+dateStr+") > 0 ")
			...
			.append(" AND i.AD_Org_ID=").append(p_AD_Org_ID);
		if (p_IsAllCurrencies)
	...
	private String createGLJournal()
	{
		...
		journal.setC_Period_ID(MPeriod.getC_Period_ID(getCtx(), p_DateReval, p_AD_Org_ID));
		...
		BigDecimal gainTotal, BigDecimal lossTotal, int AD_Org_ID, int lineNo)
	{
		...
		if (gainTotal.signum() != 0)
		{
			...
			MAccount acct = MAccount.get(getCtx(), asDefaultAccts.getAD_Client_ID(), AD_Org_ID, 
				...
				base.getUser1_ID(), base.getUser2_ID(), base.getUserElement1_ID(), base.getUserElement2_ID(), get_TrxName());
