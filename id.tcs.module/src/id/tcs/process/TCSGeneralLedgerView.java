...
public class TCSGeneralLedgerView extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		if (org.isSummary()) {
			...
			ArrayList<MOrg> orgs = getOrgList(p_AD_Org_ID);
			setOrgWhereClause(orgs);
		}
		else
	...
	protected ArrayList<MOrg> getOrgList(int AD_Org_ID) throws SQLException {
		...
		try{
			...
			while (rs.next()) {
				...
				if (org.isSummary()) {
					ArrayList<MOrg> childOrgs = getOrgList(org.get_ID());
	...
	protected void setOrgWhereClause(ArrayList<MOrg> orgs) {
		if (orgs.isEmpty()) {
			p_AD_Org_IDs = new StringBuilder();
			return ;
		}
		for (MOrg org : orgs) {
