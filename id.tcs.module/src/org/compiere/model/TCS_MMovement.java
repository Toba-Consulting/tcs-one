 ...
 */
public class TCS_MMovement extends MMovement implements DocAction
{
	/**
	 * 
	 */
	...
	public String completeIt()
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			{
				String sqlQtyDeliv = "Select qtydelivered from dd_orderline where dd_orderline_id = " + line.get_Value("dd_orderline_id");
				BigDecimal qtyDeliv = DB.getSQLValueBD(get_TrxName(), sqlQtyDeliv);	
				BigDecimal qtyEntered = (BigDecimal) line.get_Value("QtyEntered");
				BigDecimal newQtyDelivered =  qtyDeliv.add(qtyEntered);

				String sql = "UPDATE DD_OrderLine SET QtyDelivered =" + newQtyDelivered + " WHERE DD_OrderLine_ID = " + line.get_ValueAsInt("dd_orderline_id");
				String sqlMovement = "UPDATE M_MovementLine SET QtyDelivered =" + qtyEntered + " WHERE m_movementline_id = " + line.get_ValueAsInt("m_outboundline_id");
			...
			if(get_ValueAsBoolean("IsOutbound") && !isReversal())
			{
					BigDecimal newQtyDelivered =  line.getDD_OrderLine().getQtyDelivered().add((BigDecimal) line.get_Value("QtyEntered"));
					String sql = "UPDATE DD_OrderLine SET QtyDelivered =" + newQtyDelivered + " WHERE DD_OrderLine_ID = " + line.getDD_OrderLine_ID();
					String sqlMovement = "UPDATE M_MovementLine SET QtyDelivered =" + line.getDD_OrderLine().getQtyDelivered() + " WHERE m_movementline_id = " + line.getM_MovementLine_ID();
	...
	public boolean reverseCorrectIt()
	{
		...
		TCS_MMovement reversal = reverse(false);
		if (reversal == null)
			return false;
	...
	protected TCS_MMovement reverse(boolean accrual)
	{
		...
		//	Deep Copy
		TCS_MMovement reversal = new TCS_MMovement(getCtx(), 0, get_TrxName());
	...
	public boolean reverseAccrualIt()
	{
		...
		TCS_MMovement reversal = reverse(true);
		if (reversal == null)
			return false;
